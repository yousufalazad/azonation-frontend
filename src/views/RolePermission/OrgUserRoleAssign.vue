...
const getRoleTitles = async () => {
    roleTitles.value = await auth.fetchProtectedApi(
        `/api/org-role-titles?org_type_user_id=${org_type_user_id}`
    )
}

const getUsers = async () => {
    users.value = await auth.fetchProtectedApi('/api/users?type=individual')
}

const getOrganisationUsers = async () => {
    organisation_users.value = await auth.fetchProtectedApi(
        '/api/users?type=organisation'
    )
}

const getMembers = async () => {
    organisation_members.value = await auth.fetchProtectedApi(
        `/api/org-members-users/${org_type_user_id}`
    )
}
...
const saveRoleTitle = async () => {
    if (!roleTitleName.value.trim()) {
        return Swal.fire({
            ...
            text: 'Please enter a role title name.'
        })
    }

    const payload = {
        name: roleTitleName.value,
        org_type_user_id: orgId
    }

    const apiUrl = roleTitleEditMode.value
        ? `/api/org-role-titles/${roleTitleId.value}`
        : '/api/org-role-titles'

    const method = roleTitleEditMode.value ? 'PUT' : 'POST'

    const res = await Swal.fire({
        title: roleTitleEditMode.value
            ? 'Update role title?'
            : 'Create role title?',
        text: `Do you want to ${
            roleTitleEditMode.value ? 'update' : 'create'
        } "${roleTitleName.value}"?`,
        icon: 'question',
        showCancelButton: true,
        confirmButtonText: roleTitleEditMode.value
            ? 'Yes, update'
            : 'Yes, create',
    ...
    try {
        const r = await auth.fetchProtectedApi(
            apiUrl,
            payload,
            method
        )
        ...
        Toast.fire({
            icon: 'success',
            title: r.message || 'Role title saved'
        })
        ...
        Swal.fire({
            ...
            text: 'Could not save the role title. Please try again.'
        })
    } finally {
...
const deleteRoleTitle = async (id) => {
    ...
    try {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Delete failed',
            text: 'Could not delete this role title. Please try again.'
        })
    } finally {
...
const saveUserRoles = async () => {
    if (!selectedUserId.value) {
        return Swal.fire({
            ...
            text: 'Please choose a member first.'
        })
    }
    ...
    if (!org_role_title_id.value) {
        return Swal.fire({
            ...
            text: 'Please choose a role title first.'
        })
    }
    ...
    try {
        ...
        /*
         * After refreshing members, reload the selected user's
         * previous roles so the UI stays synchronized.
         */
        loadSelectedUserRoles(selectedUserId.value)
    } catch (error) {
        console.error(error)
...
const filteredOrganisationUsers = computed(() => {
    if (!userSearch.value.trim()) {
        return organisation_users.value
    }
...
/*
 * IMPORTANT:
 * Always convert member ID to String.
 *
 * SearchableSelect may return "15"
 * while API may return 15.
 *
 * String(15) === String("15") => true
 */
...
const roleTitleOptions = computed(() =>
    roleTitles.value.map(r => ({
        value: r.id,
        label: `${r.name} (${
            filteredOrganisationUsers.value.find(
                u => String(u.id) === String(r.org_type_user_id)
            )?.org_name || 'N/A'
        })`
    }))
)
...
const selectedMember = computed(() =>
    organisation_members.value.find(
        u => String(u.id) === String(selectedUserId.value)
    ) || null
)
...
/*
 * Extract role names safely from different possible API formats.
 *
 * Supported:
 *
 * roles: [
 ...
 *
 * permissions/other fields are ignored.
 */
...
/*
 * Load previous roles for selected member.
 *
 * The important part is comparing IDs using String()
 * so number/string mismatch does not break identification.
 */
const loadSelectedUserRoles = (userId) => {
    if (!userId) {
        selectedUserRoles.value = []
        return
    }

    const user = organisation_members.value.find(
        u => String(u.id) === String(userId)
    )

    selectedUserRoles.value = extractRoleNames(user)

    console.log('Selected User ID:', userId)
    console.log('Matched User:', user)
    console.log('Previous Roles:', selectedUserRoles.value)
}
...
const toggleRole = (roleName) => {
    ...
    if (idx === -1) {
        ...
        selectedUserRoles.value.splice(idx, 1)
    }
}
 ...
 * When member changes:
 *
 * 1. Find member by normalized ID
 * 2. Read previous roles
 * 3. Automatically mark those roles selected
 */
watch(
    selectedUserId,
    (newUserId) => {
        loadSelectedUserRoles(newUserId)
    },
    {
        immediate: true
    }
...
/*
 * If members are loaded/reloaded after selecting a user,
 * reload that user's roles automatically.
 */
watch(
...
<template>
    <div class="max-w-5xl mx-auto">
        ...
        >
            ...
            <!-- HEADER -->
            <div
                class="bg-gradient-to-r from-indigo-600 to-blue-600 px-6 py-5 sm:px-8 sm:py-6"
            >
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-indigo-100 text-sm mt-0.5">
                            Manage role titles and assign roles to members
                        </p>
                    </div>
            ...
            <!-- TABS -->
            <div
                class="flex gap-1 px-4 sm:px-6 pt-4 border-b border-gray-200 bg-gray-50"
            >
            ...
            <div
                v-if="isLoadingData"
                class="p-10 flex flex-col items-center justify-center text-gray-400 gap-3"
            >
                <svg
            ...
            <template v-else>
                ...
                <!-- ROLES TITLE TAB -->
                <div
                    v-if="activeTab === 'roles_title'"
                    class="p-6 sm:p-8"
                >

                    <div
                        class="bg-gray-50 border border-gray-200 rounded-xl p-4 sm:p-5 mb-6"
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                <option
                                    v-for="user in filteredOrganisationUsers"
                                    :key="user.id"
                                    :value="user.id"
                                >
                                    {{ user.org_name }}
                            ...
                            <div class="flex gap-2 shrink-0">
                                ...
                                <button
                                    v-if="roleTitleEditMode"
                                    @click="resetRoleTitleForm"
                    ...
                    >
                        <table class="min-w-full">
                            ...
                            <thead class="bg-gray-50">
                                <tr>
                                    <th
                                        class="p-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide"
                                    >
                                        Role Title
                                    </th>
                                    ...
                                    <th
                                        class="p-3.5 text-right text-xs font-semibold text-gray-500 uppercase tracking-wide"
                                    >
                                        Actions
                                    </th>
                                </tr>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                >
                                    ...
                                    <td class="p-3.5">
                                        ...
                                        >
                                            {{
                                                filteredOrganisationUsers.find(
                                                    u =>
                                                        String(u.id) ===
                                                        String(r.org_type_user_id)
                                                )?.org_name || 'N/A'
                                            }}
                                        </span>
                                    </td>
                                    ...
                                    <td
                                        class="p-3.5 text-right space-x-2 whitespace-nowrap"
                                    >

                                        <button
                                            @click="editRoleTitle(r)"
                                        ...
                                        <button
                                            @click="deleteRoleTitle(r.id)"
                                            :disabled="
                                                deletingRoleTitleId === r.id
                                            "
                                ...
                                <tr v-if="roleTitles.length === 0">
                                    ...
                                    >
                                        ...
                                        <p class="text-sm">
                                            No role titles found. Create one
                ...
                >
                    ...
                    >
                        ...
                        <div class="flex items-center gap-3 mb-5">
                            ...
                            <div
                                class="w-10 h-10 rounded-full bg-indigo-600 text-white flex items-center justify-center font-semibold text-sm shrink-0"
                            >
                                {{
                                    getInitials(
                    ...
                    >
                        ...
                        <p class="text-sm">
                            Select a member above to manage their roles.
                        </p>
