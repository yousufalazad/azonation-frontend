...
//     formData.append('image', image.value); // Add image to FormData
...
const submitForm = async () => {
    // Log payload to inspect what is being sent
    console.log({
        user_id: userId, // Ensure this is correctly defined
        title: title.value,
        image: image.value,
        history: history.value,
        status: status.value
    ...
    const payload = {
        user_id: userId, // Make sure userId is set correctly
        title: title.value,
        image: image.value,
        history: history.value,
        status: status.value
    ...
    try {
        ...
        if (result.isConfirmed) {
            ...
            if (payload.image) {
                formData.append('image', payload.image); // Handle file upload
            }
                ...
                <tbody>
                    <tr v-for="(record, index) in recordList" :key="record.id">
                        ...
                        <td class="py-2 px-4 border border-gray-300">
                            <button @click="editRecord(record)" class="bg-yellow-500 text-white rounded-md py-1 px-3 mr-2 hover:bg-yellow-600">Edit</button>
                            <button @click="deleteRecord(record.id)" class="bg-red-500 text-white rounded-md py-1 px-3 hover:bg-red-600">Delete</button>
                        </td>
                    </tr>
                </tbody>
            </table>
