...
const selectedProfile = ref(
  localStorage.getItem("history_profile") || "detailed",
);
const visibleColumns = ref(
  JSON.parse(localStorage.getItem("history_columns")) || [
    "title",
    "privacy",
    "is_active",
    "actions",
...
const filteredRecords = computed(() => {
  return historyList.value.filter((h) => {
    ...
    const matchQuick =
      quickFilter.value === "" || h.is_active === quickFilter.value;
  ...
  <div class="p-6 space-y-6 bg-white shadow rounded-lg">
    ...
    >
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <button
          @click="$router.push({ name: 'create-history' })"
    ...
    <div class="overflow-x-auto">
      ...
      >
        <template #item-history="{ history }">
          <div
            v-html="sanitize(history)"
            class="text-sm text-gray-700 max-w-[400px] line-clamp-3"
          ></div>
        </template>
        ...
        <template #item-actions="{ id }">
          <div class="flex justify-end gap-2">
            <button
              @click="$router.push({ name: 'view-history', params: { id } })"
              class="bg-white text-gray-700 hover:bg-gray-100 border border-gray-300 rounded-md py-1 px-3"
            >
              View
            </button>
            <button
              @click="$router.push({ name: 'edit-history', params: { id } })"
              class="bg-white text-gray-700 hover:bg-gray-100 border border-gray-300 rounded-md py-1 px-3"
            >
              Edit
            </button>
            <button
              @click="deleteHistory(id)"
              class="bg-white text-gray-700 hover:bg-gray-100 border border-gray-300 rounded-md py-1 px-3"
            >
              Delete
            </button>
