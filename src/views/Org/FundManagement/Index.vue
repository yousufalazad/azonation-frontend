...
const saveOrUpdateCurrency = async () => {
  ...
  try {
    ...
    if (response.status) {
      ...
      Swal.fire(
        "Failed",
        response.message || "Failed to save currency",
        "error",
      );
    }
  } catch (error) {
...
const selectedProfile = ref(
  localStorage.getItem("selected_account_profile") || "detailed",
);
const visibleColumns = ref(
  JSON.parse(localStorage.getItem("visible_account_columns")) ||
...
const submitForm = async () => {
  ...
  formData.append("fund_id", fundId.value);
  ...
  if (selectedTransactionId.value) {
    url = `/api/fund-transactions/${selectedTransactionId.value}`;
    formData.append("_method", "PUT");
  }

  const result = await Swal.fire({
    title: "Are you sure?",
    text: `Do you want to ${selectedTransactionId.value ? "update" : "add"} this fund transaction?`,
    icon: "warning",
...
<template>
  <div class="p-4 sm:p-6 bg-white rounded-lg shadow space-y-6">
      ...
      <table class="min-w-full border border-gray-200 text-sm">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th
              ...
              @click="sort('transaction_title')"
              class="cursor-pointer px-3 py-2 text-left font-bold text-gray-700 hover:text-blue-600"
            >
              Title
              <span v-if="sortBy === 'transaction_title'">{{
                sortDirection === "asc" ? "▲" : "▼"
              }}</span>
            </th>

            <th
              v-if="visibleColumns.includes('fund')"
              @click="sort('fund')"
              class="cursor-pointer px-3 py-2 text-left font-bold text-gray-700 hover:text-blue-600"
            >
              Fund
              <span v-if="sortBy === 'fund'">{{
                sortDirection === "asc" ? "▲" : "▼"
              }}</span>
            </th>
            ...
            <th
              v-if="visibleColumns.includes('expense')"
              @click="sort('amount')"
              class="cursor-pointer px-3 py-2 text-left font-bold text-gray-700 hover:text-blue-600"
            >
        ...
        <tbody class="bg-white divide-y divide-gray-100">
          ...
          >
            ...
            <td v-if="visibleColumns.includes('date')" class="px-3 py-2">
              {{ transaction.date }}
            </td>
            <td v-if="visibleColumns.includes('title')" class="px-3 py-2">
              {{ transaction.transaction_title }}
            </td>
            <td v-if="visibleColumns.includes('fund')" class="px-3 py-2">
              {{ transaction.funds?.name }}
            </td>
            <td
              v-if="visibleColumns.includes('income')"
              class="px-3 py-2 text-green-600 font-medium"
            >
            ...
            <td
              v-if="visibleColumns.includes('expense')"
              class="px-3 py-2 text-red-600 font-medium"
            >
    ...
    >
      <!-- Info -->
      <div
        class="text-sm text-gray-600 text-center md:text-left w-full md:w-auto"
      >
      ...
      <!-- Controls -->
      <div
        class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto justify-center"
      >
        <!-- Items per page -->
        <div class="flex items-center gap-2">
          ...
          >
            <option
              v-for="size in [5, 10, 20, 50, 100, 250, 500, 1000]"
              :key="size"
              :value="size"
            >
              {{ size }}
      ...
      >
        ...
        <div class="space-y-4 text-gray-700">
          ...
          <div class="flex justify-between border-b pb-2">
            ...
            <span class="font-semibold">{{
              selectedTransaction.funds?.name || "N/A"
            }}</span>
          </div>
          <div class="flex justify-between border-b pb-2">
          ...
          <!-- Attached Images -->
          <div
            v-if="selectedTransaction.images?.length"
            class="border-t mt-6 pt-4"
          >
          ...
          <!-- Attached Documents -->
          <div
            v-if="selectedTransaction.documents?.length"
            class="border-t mt-6 pt-4"
          >
            <h3 class="text-lg font-semibold text-gray-800 mb-3">
              Attached Documents
            ...
            <ul class="space-y-2 text-blue-600">
              <li
                v-for="doc in selectedTransaction.documents"
                :key="doc.id"
    ...
    >
      <div
        class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-2xl relative overflow-y-auto max-h-[90vh]"
      >
        <button
          @click="closeModal"
        ...
        <h2 class="text-2xl font-semibold text-gray-800 mb-6">
          {{ selectedTransactionId ? "Edit" : "Add" }} Fund Transaction
        </h2>
        <form @submit.prevent="submitForm" class="space-y-4">
          ...
          <div>
            ...
            <div class="space-y-3">
              <div
                v-for="(file, index) in modalImages"
                :key="file.id"
              ...
              >
                <input
                  ...
                  accept="image/*"
                  @change="
                    (event) => handleFileChange(event, modalImages, index)
                  "
                ...
                >
                  ...
                  <button
                    type="button"
                    @click="removeFile(modalImages, index)"
              ...
              <button
                type="button"
                @click="() => addMoreFiles(modalImages)"
          ...
          <div>
            ...
            <div class="space-y-3">
              <div
                v-for="(file, index) in modalDocuments"
                :key="file.id"
                class="flex flex-col sm:flex-row sm:items-center gap-4"
              >
                <input
                  ...
                  accept=".pdf,.doc,.docx"
                  @change="
                    (event) => handleFileChange(event, modalDocuments, index)
                  "
                  class="w-full sm:w-auto border border-gray-300 rounded-md px-3 py-2"
                />
                ...
                <div v-if="file.file" class="flex items-center gap-3">
                  ...
                  <button
                    type="button"
                    @click="removeFile(modalDocuments, index)"
                    class="bg-red-500 hover:bg-red-600 text-white font-semibold text-sm px-2 py-1 rounded"
                  >
                    X
                  </button>
              ...
              <button
                type="button"
                @click="() => addMoreFiles(modalDocuments)"
                class="text-blue-600 hover:underline text-sm"
              >
                + Add more document
              </button>
