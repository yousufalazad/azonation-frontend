<template>
    <div class="profile-data-show">
      ...
      <div class="space-y-4">
        <p>Short Description: <span>{{ shortDescription }}</span> <button @click="openModal('shortDescription')" class="text-blue-500">Edit</button></p>
        <p>Detail Description: <span>{{ detailDescription }}</span> <button @click="openModal('detailDescription')" class="text-blue-500">Edit</button></p>
        <p>Mission: <span>{{ mission }}</span> <button @click="openModal('mission')" class="text-blue-500">Edit</button></p>
        <p>Vision: <span>{{ vision }}</span> <button @click="openModal('vision')" class="text-blue-500">Edit</button></p>
        <p>Value: <span>{{ value }}</span> <button @click="openModal('value')" class="text-blue-500">Edit</button></p>
        <p>Areas of Focus: <span>{{ areasOfFocus }}</span> <button @click="openModal('areasOfFocus')" class="text-blue-500">Edit</button></p>
        <p>Causes: <span>{{ causes }}</span> <button @click="openModal('causes')" class="text-blue-500">Edit</button></p>
        <p>Impact: <span>{{ impact }}</span> <button @click="openModal('impact')" class="text-blue-500">Edit</button></p>
        <p>Scope of Work: <span>{{ scopeOfWork }}</span> <button @click="openModal('scopeOfWork')" class="text-blue-500">Edit</button></p>
        <p>Organising Date: <span>{{ organisingDate }}</span> <button @click="openModal('organisingDate')" class="text-blue-500">Edit</button></p>
        <p>Foundation Date: <span>{{ foundationDate }}</span> <button @click="openModal('foundationDate')" class="text-blue-500">Edit</button></p>
        <p>Status: <span>{{ status }}</span> <button @click="openModal('status')" class="text-blue-500">Edit</button></p>
      </div>
  ...
  const openModal = (field) => {
      ...
      isTextarea.value = (field === 'shortDescription' || field === 'detailDescription');
