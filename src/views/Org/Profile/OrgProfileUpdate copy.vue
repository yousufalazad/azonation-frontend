...
const fetchOrgDetails = async () => {
    try {
        const response = await auth.fetchProtectedApi(`/api/organisation/${userId}`, {}, 'GET');
        if (response.status) {
...
const updateOrgProfile = async () => {
    try {
        const response = await auth.fetchProtectedApi(`/api/org-profile/${userId}`, {
...
<template>
    ...
    <div>
        ...
        <p>Org detail description: <span>{{ detailDescription }}</span></p>
        <p>Org primary ID: <span>{{ primaryId }}</span></p>
        <p>Org user ID: <span>{{ userId }}</span></p>
        <p>Org user email: <span>{{ userEmail }}</span></p>
    ...
    <div class="card shadow-sm">
        <div class="card-body p-4">
            ...
            <div class="mb-3">
                ...
                <textarea id="detailDescription" v-model="detailDescription" class="form-control" placeholder="Enter detail description"></textarea>
            </div>
            
            <div class="text-end">
                <button @click="updateOrgProfile" class="btn btn-primary">Update profile</button>
            </div>
        </div>
    </div>
<br>
