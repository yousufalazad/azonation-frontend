...
const getAssetDetails = async () => {
    try {
        ...
        if (response.status) {
            ...
            is_long_term.value = asset.is_long_term ? 1 : 0;
            quantity.value = asset.quantity;
            value_amount.value = asset.value_amount;
            inkind_value.value = asset.inkind_value;
            is_tangible.value = asset.is_tangible ? 1 : 0;
            responsible_user_id.value = asset.responsible_user_id;
            assignment_start_date.value = asset.assignment_start_date;
            assignment_end_date.value = asset.assignment_end_date;
            note.value = asset.note;
            asset_lifecycle_statuses_id.value = asset.asset_lifecycle_statuses_id;
            privacy_setup_id.value = asset.privacy_setup_id;
            is_active.value = asset.is_active ? 1 : 0;
        }
    } catch (error) {
...
const submitForm = async () => {
    ...
    try {
        ...
        if (result.isConfirmed) {
            const response = await auth.fetchProtectedApi(`/api/update-asset/${assetId}`, payload, 'PUT');
            if (response.status) {
                Swal.fire('Success!', 'Event update successfully.', 'success').then(() => {
