...
const selectedProfile = ref(
  localStorage.getItem("yearplan_profile") || "detailed",
);
...
const filteredRecords = computed(() =>
  recordList.value.filter((record) => {
    ...
    const matchQuick =
      quickFilter.value === "" ||
      String(record.is_active) === quickFilter.value;
    const matchStart =
      startDate.value === "" ||
  ...
  <div class="p-6 space-y-6 bg-white shadow rounded-lg">
    ...
    >
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <button
          @click="$router.push({ name: 'create-year-plan' })"
      ...
      >
        ...
        <template #item-actions="{ id }">
          <div class="flex justify-end flex-wrap gap-2">
            <button
              @click="$router.push({ name: 'view-year-plan', params: { id } })"
              class="bg-white text-gray-700 hover:bg-gray-100 border rounded-md py-1 px-3"
            >
              View
            </button>
            <button
              @click="$router.push({ name: 'edit-year-plan', params: { id } })"
              class="bg-white text-gray-700 hover:bg-gray-100 border rounded-md py-1 px-3"
            >
              Edit
            </button>
