...
const selectedProfile = ref(
  localStorage.getItem("selected_committee_profile") || "detailed",
);
const visibleColumns = ref(
  JSON.parse(localStorage.getItem("visible_committee_columns")) ||
...
const filteredCommittees = computed(() => {
  ...
  if (startFilter.value)
    list = list.filter((c) => c.start_date >= startFilter.value);
...
const totalPages = computed(() =>
  Math.max(1, Math.ceil(totalItems.value / rowsPerPage.value)),
);
...
watch([search, startFilter, endFilter], () => {
  currentPage.value = 1;
});
...
const applyQuickDateFilter = () => {
  ...
  if (quickDateFilter.value === "last7") {
    ...
    startFilter.value = format(d);
    endFilter.value = format(today);
    ...
    startFilter.value = format(d1);
    endFilter.value = format(d2);
  } else {
    startFilter.value = "";
    endFilter.value = "";
  }
...
const runExport = async (type, exportFn) => {
  if (!filteredCommittees.value.length) {
    return Swal.fire({
      ...
      text: "There are no committees matching the current filters.",
    });
  }
...
const fetchCommitteeList = async () => {
  ...
  try {
    ...
    committeeList.value = res.status
      ? res.data.map((c) => ({
          id: c.id,
          name: c.name,
  ...
  try {
    ...
    if (response.status) {
      ...
      Swal.fire({
        icon: "error",
        title: "Delete failed",
        text: "Could not delete this committee.",
      });
    }
  } catch (error) {
...
const saveCommittee = async () => {
  if (!newName.value.trim()) {
    return Swal.fire({
      ...
      text: "Please enter a name for the committee.",
    });
  }
  ...
  try {
    ...
    Toast.fire({
      icon: "success",
      title: isEditMode.value ? "Committee updated" : "Committee created",
    });
    closeModal();
    await fetchCommitteeList();
  } catch (error) {
    ...
    Swal.fire({
      ...
      text: "Could not save the committee. Please try again.",
    });
  } finally {
  ...
  <div class="p-4 sm:p-6 bg-white shadow rounded-lg space-y-6">
    ...
    >
      ...
      <div class="flex flex-wrap gap-2 w-full sm:w-auto">
        ...
        <button
          @click="exportXLSX"
          :disabled="exportingType === 'xlsx'"
          class="flex items-center gap-1.5 border border-gray-300 bg-white px-3 py-1.5 text-sm rounded text-gray-700 hover:bg-gray-100 transition disabled:opacity-60"
        >
          <svg
            v-if="exportingType === 'xlsx'"
      ...
      >
        ...
        <template #item-actions="{ id }">
          <div class="flex flex-wrap justify-end gap-2">
            <button
              v-if="auth.hasPermission('committee-member.read')"
              @click="
                $router.push({
                  name: 'index-committee-member',
                  params: { committeeId: id },
                })
              "
            ...
            <button
              ...
              :disabled="deletingCommitteeId === id"
      ...
      <div class="flex flex-col sm:flex-row items-center gap-3">
        ...
        <div class="flex flex-wrap justify-center gap-1">
          <button
            ...
            class="border rounded px-3 py-1 text-sm transition"
    ...
    >
      ...
      >
        ...
        <div
          :class="[
            'px-6 md:px-8 py-5 flex items-center gap-3',
            isEditMode ? 'bg-amber-50' : 'bg-blue-50',
          ]"
        >
          <div
            :class="[
          ...
          <div>
            ...
            <p class="text-xs text-gray-500 mt-0.5">
              {{
                isEditMode
                  ? "Update the committee details below"
        ...
        <!-- Footer -->
        <div
          class="flex justify-end gap-3 px-6 md:px-8 py-4 bg-gray-50 border-t"
        >
          <button
            @click="closeModal"
    ...
    >
      ...
      >
        ...
        <div
          class="bg-gradient-to-r from-indigo-600 to-blue-600 px-6 md:px-8 py-6"
        >
          <div class="flex items-center gap-3">
            <div
