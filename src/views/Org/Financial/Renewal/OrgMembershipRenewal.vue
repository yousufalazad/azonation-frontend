...
const allHeaders = [
  ...
  { text: "Membership ID", value: "existing_membership_id", sortable: true },
  {
    ...
    value: "membership_type_before_termination",
    sortable: true,
  },
...
const selectedProfile = ref(
  localStorage.getItem("selected_renewal_profile") || "detailed",
);
const visibleColumns = ref(
  JSON.parse(localStorage.getItem("visible_renewal_columns")) ||
...
const filteredList = computed(() => {
  ...
  let list = (renewals.value || []).map((m) => {
    ...
    try {
      parsedMoreInfo =
        typeof m.more_info === "string"
          ? JSON.parse(m.more_info)
          : m.more_info || {};
    } catch {
      parsedMoreInfo = {};
    }
    ...
    return {
      ...m,
      full_name: m.individual
        ? `${m.individual.first_name || ""} ${m.individual.last_name || ""}`.trim()
        : m.full_name ||
          `${parsedMoreInfo.first_name || ""} ${parsedMoreInfo.last_name || ""}`.trim(),
      image_url: m.image_url ?? placeholderImage,
      existing_membership_id:
        parsedMoreInfo.existing_membership_id ||
  ...
  if (kw) {
    list = list.filter((m) => {
      ...
      const membershipId = (m.existing_membership_id || "")
        .toString()
        .toLowerCase();
      const membershipType = (m.membership_type_before_termination || "")
        .toString()
        .toLowerCase();
      ...
      return (
        ...
        membershipId.includes(kw) ||
        membershipType.includes(kw) ||
        invoice.includes(kw) ||
        payment.includes(kw)
      );
    });
  ...
  if (membershipTypeFilter.value) {
    list = list.filter(
      (m) =>
        (m.membership_type_before_termination || "") ===
...
const totalPages = computed(() =>
  Math.max(1, Math.ceil(filteredCount.value / rowsPerPage.value)),
);
...
const fetchRenewalCycles = async () => {
  try {
    const res = await auth.fetchProtectedApi(
      "/api/org-membership-renewal-cycles",
      {},
      "GET",
    );
...
const openModalForCreate = () => {
  ...
  Object.keys(form.value).forEach(
    (k) =>
      (form.value[k] = k === "attempt_count" ? 0 : k === "is_active" ? 1 : ""),
  );
  ...
  try {
    ...
    if (isEditMode.value && form.value.id) {
      res = await auth.fetchProtectedApi(
        `/api/org-membership-renewals/${form.value.id}`,
        form.value,
        "PUT",
      );
    } else {
...
const deleteRenewal = async (id) => {
  ...
  try {
    const res = await auth.fetchProtectedApi(
      `/api/org-membership-renewals/${id}`,
      {},
      "DELETE",
    );
    if (res.status) {
...
const exportPDF = async () => {
  ...
  doc.save(
    `Org_Membership_Renewals_${new Date().toISOString().slice(0, 10)}.pdf`,
  );
};
...
const exportDOCX = async () => {
    ...
    new TableRow({
      children: visibleHeaders.value.map(
        (h) =>
          new TableCell({
            width: {
              size: Math.floor(100 / visibleHeaders.value.length),
          ...
          children: visibleHeaders.value.map(
            (h) =>
              new TableCell({
                children: [
                  new Paragraph({
                    children: [new TextRun(String(item[h.value] ?? ""))],
                  }),
                ],
              }),
          ),
    ...
    sections: [
      {
        ...
        children: [
          new Paragraph({
            children: [
              new TextRun({
                text: "Organization Membership Renewals",
                bold: true,
                size: 24,
              }),
            ],
  ...
  saveAs(
    blob,
    `Org_Membership_Renewals_${new Date().toISOString().slice(0, 10)}.docx`,
  );
};
...
<template>
  <div class="p-6 bg-white rounded-lg shadow space-y-6">
    ...
    <div :class="showFilters ? 'block' : 'hidden'" class="sm:block space-y-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <div>
          ...
          <input
            v-model="search"
            type="text"
            placeholder="Search by name, invoice, payment id..."
        ...
        <div>
          ...
          >
            ...
            <option v-for="t in membershipTypes" :key="t.id" :value="t.name">
              {{ t.name }}
            </option>
          </select>
        </div>
      ...
      <div
        class="bg-gray-50 border rounded p-4 flex flex-col lg:flex-row gap-6 items-start"
      >
        <div>
        ...
        <div class="flex-1">
          ...
          <div class="flex flex-wrap gap-4">
            ...
            >
              ...
              />
              <label :for="'col_' + h.value" class="text-gray-700">{{
                h.text
              }}</label>
            </div>
          </div>
    ...
    <div class="overflow-x-auto">
      ...
      >
        ...
        <template
          #item-membership_type_before_termination="{
            membership_type_before_termination,
          }"
        >
        ...
        <template #item-actions="{ id }">
          <div class="flex gap-2 justify-center">
            <button
              @click="viewRenewal(findById(id))"
              class="text-blue-600 hover:underline text-xs"
            >
              View
            </button>
            <button
              @click="editRenewal(findById(id))"
              class="text-green-600 hover:underline text-xs"
            >
              Edit
            </button>
            <button
              @click="deleteRenewal(id)"
    ...
    >
      <div class="text-xs sm:text-sm text-gray-600 text-center md:text-left">
        ...
        {{ filteredCount }} | Page {{ currentPage }} of {{ totalPages }}
      </div>
      ...
      <div class="flex items-center gap-3">
        <div class="flex items-center gap-2">
          ...
          >
            <option v-for="s in [5, 10, 20, 50, 100]" :key="s" :value="s">
              {{ s }}
            </option>
          </select>
        </div>
        ...
        <form @submit.prevent="saveRenewal" class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                v-model="form.individual_type_user_id"
                class="w-full border rounded-lg px-3 py-2"
              >
                <option value="">Select</option>
                <option
                  v-for="i in individualTypes"
                  :key="i.individual_type_user_id"
            ...
            <div>
              <label class="block text-sm font-medium"
                >Membership Renewal Cycle</label
              >
              <select
                v-model="form.membership_renewal_cycle_id"
                class="w-full border rounded-lg px-3 py-2"
              >
                <option value="">Select</option>
                <option
                  v-for="c in renewalCycles"
                  :key="c.member_renewal_cycle_id"
                  :value="c.member_renewal_cycle_id"
                >
      ...
      >
        ...
        <div class="grid grid-cols-1 gap-y-4 text-sm text-gray-700">
          <div class="flex justify-between">
            ...
            ><span>{{ selectedMemberView.full_name || "—" }}</span>
          </div>
          <div class="flex justify-between">
            <span class="font-medium text-gray-600">Membership Type</span
            ><span>{{
              selectedMemberView.membership_type_before_termination || "—"
            }}</span>
          </div>
          <div class="flex justify-between">
            <span class="font-medium text-gray-600">Amount Paid</span
            ><span>{{ selectedMemberView.amount_paid ?? "—" }}</span>
          </div>
          <div class="flex justify-between">
            <span class="font-medium text-gray-600">Status</span
            ><span>{{ selectedMemberView.status || "—" }}</span>
          </div>
          <div class="flex justify-between">
            <span class="font-medium text-gray-600">Renewed At</span
            ><span>{{ formatDateTime(selectedMemberView.renewed_at) }}</span>
          </div>
          <div class="flex justify-between">
            <span class="font-medium text-gray-600">Org Notes</span
            ><span class="whitespace-pre-wrap">{{
              selectedMemberView.org_notes || "—"
            }}</span>
          </div>
        </div>
