      ...
      <div v-if="searchResults.length" class="results-container">
        <ul class="list-none">
          <li v-for="individualUser in searchResults" :key="individualUser.id" class="flex justify-between items-center py-2 border-b">
            <img :src="`${baseURL}/storage/${individualUser.image}`" alt="Profile picture" class="w-11 h-11 rounded-full object-covers"> {{ individualUser.name }}, {{ individualUser.email }}, Mobile: {{ individualUser.dialing_code }}{{ individualUser.phone_number }}, City: {{ individualUser.city }}, Country: {{ individualUser.country_name }}, Azon Id: {{ individualUser.azon_id }} 
