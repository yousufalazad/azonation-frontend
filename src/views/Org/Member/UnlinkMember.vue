...
const filteredMembers = computed(() => {
  ...
  return members.value.filter((m) =>
    [m.first_name, m.last_name, m.email, m.mobile].some((field) =>
...
const saveMember = async () => {
  ...
  formData.append(
    "existing_membership_id",
    form.value.existing_membership_id ?? "",
  );
  formData.append("membership_type_id", form.value.membership_type_id ?? "");
  formData.append(
    "membership_status_id",
    form.value.membership_status_id ?? "",
  );
  formData.append(
    "membership_start_date",
    form.value.membership_start_date ?? "",
  );
  ...
  try {
    ...
    if (res.status) {
      Swal.fire(
        "Success",
        `Member ${isEditMode.value ? "updated" : "created"} successfully.`,
        "success",
      );
      fetchMembers();
      closeModal();
...
const exportHeaders = computed(() => {
  const selectedColumns =
    columnView.value === "minimal" ? minimalColumns : detailedColumns;
...
<template>
  <div class="card p-6 bg-white rounded-lg shadow-md">
    ...
    <div class="flex flex-wrap justify-between items-start gap-4 py-4">
      <div class="flex flex-col gap-1">
        <h2 class="text-lg font-semibold text-gray-700">
          Unlinked Members <span class="text-red-500">(Not Recommended)</span>
        </h2>
        <p class="text-xs text-gray-500 max-w-2xl break-words">
          It is recommended to add members using the
          <span class="font-medium text-gray-600">Search</span> option. If the
          member does not have an Azonation account, you can add them manually.
          Please note that
          <span class="font-semibold text-red-500">unlinked members</span> will
          not receive updates, and no actions can be performed for them within
          Azonation. This list is for record purposes only.
        </p>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <button
          @click="exportCSV"
          class="px-3 py-2 border border-gray-300 rounded-md text-sm hover:bg-gray-100"
        >
          CSV
        </button>
        <button
    ...
    <button
      ...
      type="button"
      class="sm:hidden w-full flex items-center justify-between border rounded-lg px-4 py-2.5 my-4 bg-gray-50 text-sm font-medium text-gray-700"
    >
    ...
    <div :class="showFilters ? 'block' : 'hidden'" class="sm:block space-y-4">
      ...
      <div
        class="border rounded bg-gray-50 p-4 flex flex-wrap gap-6 items-center mb-5"
      >
    ...
    <div class="overflow-x-auto custom-scrollbar">
      <table class="w-full min-w-[800px] divide-y divide-gray-200 border">
        <thead class="bg-gray-50">
          <tr>
            <th class="py-3 px-4 text-left text-xs font-bold text-gray-600">
              #
            </th>

            <!-- Image -->
            <th
              v-if="visibleColumns.image"
              class="py-3 text-left text-xs font-bold text-gray-600"
            >
              Image
            </th>
            ...
            <!-- Email with Sorting -->
            <th
              v-if="visibleColumns.email && columnView === 'detailed'"
              @click="sort('email')"
            ...
            <!-- Address -->
            <th
              v-if="visibleColumns.address && columnView === 'detailed'"
              @click="sort('address')"
              class="cursor-pointer py-3 text-left text-xs font-bold hover:text-blue-600"
            >
              Address
            ...
            <!-- Note -->
            <th
              v-if="visibleColumns.note && columnView === 'detailed'"
              class="py-3 text-left text-xs font-bold"
            >
              Note
            </th>
    ...
    >
      ...
      >
        <label for="itemsPerPage" class="whitespace-nowrap"
    ...
    >
      ...
      <div
        class="bg-white p-4 sm:p-6 rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto"
      >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            >
            <input
              v-model="form.existing_membership_id"
              type="text"
          ...
          <div>
            ...
            >
            <input
              v-model="form.membership_start_date"
              type="date"
              class="w-full mt-1 border border-gray-300 rounded-lg px-3 py-2 text-sm"
            />
          </div>
    ...
    >
      ...
      >
          ...
          <table class="w-full text-sm text-left text-gray-700">
            <tbody>
              ...
              <tr v-if="selectedMember.username">
                <th class="py-2 pr-2 font-medium text-gray-600 w-40">
                  Username
                </th>
                <th class="py-2 pr-2 font-medium text-gray-600 w-3">:</th>
                <td class="py-2 text-gray-800">
        ...
        <div class="mt-6 flex justify-end gap-3">
          <button
            @click="openModal(selectedMember)"
