    ...
    memberList.value = response.status
      ? response.data.map((m) => {
          ...
          return {
            ...
            existing_membership_id:
              parsedMoreInfo.existing_membership_id || "--", // ✅ Extracted field
...
const filteredMembers = computed(() => {
  ...
  if (membership_type_id.value) {
    list = list.filter(
      (m) => m.membership_type_before_termination === membership_type_id.value,
    );
  }
  ...
  <div class="p-6 bg-white rounded-lg shadow space-y-6">
    ...
    >
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <button
          @click="$router.push({ name: 'index-member' })"
          class="flex items-center gap-1 border border-gray-300 bg-white px-3 py-1.5 text-sm rounded text-gray-700 hover:bg-gray-100"
        >
          Present Member
        </button>
    ...
    <div class="overflow-x-auto">
      ...
      >
        ...
        <template #item-joined_at="{ joined_at }">
          <span>{{
            joined_at ? dayjs(joined_at).format("DD-MM-YYYY") : "--"
          }}</span>
        </template>
        <template #item-terminated_at="{ terminated_at }">
          <span>{{
            terminated_at ? dayjs(terminated_at).format("DD-MM-YYYY") : "--"
          }}</span>
        </template>
        <!-- Actions -->
      ...
      >
        ...
        <div class="grid grid-cols-1 gap-y-4 text-sm text-gray-700">
          ...
          <div class="flex justify-between">
            ...
            <span>{{
              selectedMember?.membership_type_before_termination ?? "--"
            }}</span>
          </div>
          <div class="flex justify-between">
          ...
          <div
            ...
            v-if="selectedMember?.membership_duration_days"
          >
            ...
            <span>
              {{
                selectedMember?.membership_termination_reason_id
                  ? membershipTerminationReasons.find(
                      (m) =>
                        m.id ===
                        selectedMember.membership_termination_reason_id,
                    ).reason
