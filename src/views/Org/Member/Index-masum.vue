...
const membership_type_edit = ref('')
...
const headers = computed(() => allHeaders.filter(h => visibleColumns.value.includes(h.value)))
...
const totalItems = computed(() => filteredHeaders.value.length)
...
const paginatedMembers = computed(() => {
  ...
  return filteredHeaders.value.slice(start, start + rowsPerPage.value)
})
...
const exportToExcel = () => {
  const data = filteredHeaders.value.map(m => ({
...
const exportToPDF = () => {
  const doc = new jsPDF()
  const rows = filteredHeaders.value.map(m => [
    m.full_name,
    m.membership_type?.name || '--',
    m.membership_start_date ? dayjs(m.membership_start_date).format('YYYY-MM-DD') : '--',
    m.existing_membership_id || '--',
...
const exportToWord = async () => {
  const tableRows = [
    new TableRow({
      children: ['Name', 'Membership Type', 'Membership ID', 'Joining Date', 'Membership Age'].map(
        header =>
          new TableCell({
  ...
  const doc = new Document({
    sections: [
      {
        children: [
          new Paragraph({ text: 'Members', heading: 'Heading1' }),
...
<template>
  <div class="p-6 bg-white rounded-lg shadow space-y-6">
    ...
    <div class="flex justify-between items-center">
      ...
      <div class="flex gap-2">
        ...
        <button @click="exportToWord"
          class="border px-3 py-1.5 text-sm rounded text-gray-700 hover:bg-gray-100">Word</button>
    ...
    <EasyDataTable
      ...
      :loading="loading"
      show-index
      :rows-per-page-options="[5, 10, 50, 100, 250, 500, 1000]"
