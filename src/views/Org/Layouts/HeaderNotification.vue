...
<template>
  <div class="relative">
    ...
    <!-- Mobile backdrop (dims page, tap to close) -->
    <transition name="fade">
      <div v-if="isDropdownOpen" class="fixed inset-0 z-40 bg-black/20 sm:hidden" @click="isDropdownOpen = false" />
    </transition>

    <!-- Dropdown: fixed centered panel on mobile, absolute anchored dropdown from sm: up -->
    <transition name="fade">
      <div v-if="isDropdownOpen" ref="dropdownRef" class="fixed left-4 right-4 top-16 z-50 max-h-[75vh] overflow-y-auto bg-white shadow-lg rounded-xl
        ...
        <div class="px-4 pt-3 pb-2 border-b">
          <div class="flex items-center justify-between gap-2">
            ...
            <div class="flex items-center gap-3 sm:gap-4 flex-wrap justify-end">
              <a href="/org-dashboard/notifications" class="text-xs text-blue-600 hover:underline">See all</a>
              <button v-if="unreadCount > 0" @click="markAllAsRead" class="text-xs text-blue-600 hover:underline whitespace-nowrap">
                Mark all as read
              </button>
            </div>
          </div>
            ...
            <a href="#" @click.prevent="markAsRead(n.id)" class="flex items-start gap-3 px-3 py-2">
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  ...
                  <span v-if="!n.read_at" class="ml-auto w-2.5 h-2.5 rounded-full bg-blue-600 shrink-0"
                    aria-hidden="true" />
                </div>
