...
<template>
    <div class="max-w-7xl mx-auto w-11/12">
        ...
        <div class="flex justify-between left-color-shade py-2 my-3">
            <h5 class="text-md mt-2">Project: {{ projectDetails.name }} - Date: {{ projectDetails.date }} - Time: {{ projectDetails.time }}</h5>
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
            <div class="bg-white rounded-xl shadow-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6">
                <div class="flex justify-between items-center border-b pb-3 mb-4">
                    ...
                    <button @click="closeModal" class="text-gray-500 hover:text-gray-800 font-bold">&times;</button>
                </div>
                <form @submit.prevent="submitForm" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                        <div class="col-span-6">
                            <label class="block text-gray-700 font-semibold mb-1">Guest Name</label>
                            <input v-model="guest_name" type="text" class="w-full border border-gray-300 rounded-md py-2 px-3" required />
                        </div>
                        <div class="col-span-6">
                            <label class="block text-gray-700 font-semibold mb-1">About Guest</label>
                            <input v-model="about_guest" type="text" class="w-full border border-gray-300 rounded-md py-2 px-3" />
                        </div>
                        <div class="col-span-6">
                            <label class="block text-gray-700 font-semibold mb-1">Attendance Type</label>
                            <select v-model="attendance_type_id" class="w-full border border-gray-300 rounded-md p-2" required>
                                <option value="">Select Attendance Type</option>
                                <option v-for="type in attendanceTypeList" :key="type.id" :value="type.id">{{ type.name }}</option>
                            </select>
                        </div>
                        <div class="col-span-3">
                            <label class="block text-gray-700 font-semibold mb-1">Date</label>
                            <input v-model="date" type="date" class="w-full border border-gray-300 rounded-md py-2 px-3" />
                        </div>
                        <div class="col-span-3">
                            <label class="block text-gray-700 font-semibold mb-1">Time</label>
                            <input v-model="time" type="time" class="w-full border border-gray-300 rounded-md py-2 px-3" />
                        </div>
                        <div class="col-span-6">
                            <label class="block text-gray-700 font-semibold mb-1">Note</label>
                            <input v-model="note" type="text" class="w-full border border-gray-300 rounded-md py-2 px-3" />
                        </div>
                        <div class="col-span-6">
                        ...
                        <div class="col-span-12 flex justify-end gap-3 mt-2">
                            <button type="submit" class="bg-green-600 text-white rounded-md py-2 px-4 hover:bg-green-500">{{ isEditMode ? 'Update' : 'Add' }}</button>
                            <button type="button" @click="resetForm" class="bg-yellow-600 text-white rounded-md py-2 px-4 hover:bg-yellow-700">Reset</button>
                            <button type="button" @click="closeModal" class="bg-gray-500 text-white rounded-md py-2 px-4 hover:bg-gray-600">Cancel</button>
                        </div>
                    </div>
                </form>
            </div>
                ...
                <tbody>
                    <tr v-for="(attendance, index) in projectGuestAttendanceList" :key="attendance.id">
                        ...
                        <td class="py-2 px-4 border flex gap-2">
                            <button @click="openModal(attendance)" class="bg-yellow-400 text-white rounded-md py-1 px-2 hover:bg-yellow-500">Edit</button>
                            <button @click="deleteMeetingAttendance(attendance.id)" class="bg-red-600 text-white rounded-md py-1 px-2 hover:bg-red-700">Delete</button>
                        </td>
                    </tr>
                </tbody>
            </table>
