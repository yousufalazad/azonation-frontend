...
const updateMeeting = async () => {
  try {
    ...
    if (response.status) {
      ...
      Swal.fire('Failed', response.errors?.message || 'Failed to update meeting.', 'error');
    }
  } catch (error) {
...
<template>
  <div class="container mx-auto max-w-5xl p-6 bg-white shadow-md rounded-lg mt-10">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
      <h5 class="text-xl font-semibold text-center sm:text-left">Edit Meeting</h5>
      <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
        <button @click="router.push({ name: 'view-individual-meeting', params: { id: selectedRecordId } })" class="btn-primary mr-2 w-full sm:w-auto">Meeting View</button>
    ...
    <form @submit.prevent="updateMeeting">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-600">Privacy Setup <span class="text-red-500">*</span></label>
        ...
        <div class="space-y-3">
          <div v-for="(file, index) in images" :key="file.id" class="flex flex-col md:flex-row gap-4">
            ...
            <div class="flex items-center gap-3">
              ...
              <button type="button" class="bg-red-500 text-white px-2 py-1 text-sm hover:bg-red-600" @click="removeFile(images, index)">X</button>
            </div>
          </div>
        </div>
        <button type="button" class="mt-3 w-full md:w-auto bg-blue-500 text-white py-1 px-3 rounded-md hover:bg-blue-700" @click="() => addMoreFiles(images)">Add more image</button>
      </div>
      ...
      <div class="mb-4">
        ...
        <div class="space-y-3">
          <div v-for="(file, index) in documents" :key="file.id" class="flex flex-col md:flex-row gap-4">
            <input type="file" class="w-full md:w-auto border border-gray-300 rounded-md py-2 px-4" accept=".pdf,.doc,.docx"
              @change="event => handleFileChange(event, documents, index)" />
            <div class="flex items-center gap-3 w-full md:w-auto">
              <span v-if="file.file" class="w-full md:w-32 truncate">{{ file.file.name }}</span>
              <button type="button" class="bg-red-500 text-white px-2 py-1 text-sm hover:bg-red-600" @click="removeFile(documents, index)">X</button>
            </div>
          </div>
        </div>
        <button type="button" class="mt-3 w-full md:w-auto bg-blue-500 text-white py-1 px-3 rounded-md hover:bg-blue-700" @click="() => addMoreFiles(documents)">Add more document</button>
      </div>
