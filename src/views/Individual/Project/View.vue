...
<template>
  <div class="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
      <h2 class="text-2xl font-bold text-gray-800 text-center sm:text-left">View Project</h2>
      <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
        <button @click="$router.push({ name: 'edit-individual-project', params: { id: projectId } })"
    ...
    <table class="table-auto w-full border-collapse border border-gray-300 text-sm">
      <tbody>
        <tr><td class="py-2 px-4 border-b font-medium">Project Title</td><td>:</td><td class="py-2 px-4 border-b">{{ project.title || 'N/A' }}</td></tr>
        <tr><td class="py-2 px-4 border-b font-medium">Venue Name</td><td>:</td><td class="py-2 px-4 border-b">{{ project.venue_name || 'N/A' }}</td></tr>
        <tr><td class="py-2 px-4 border-b font-medium">Venue Address</td><td>:</td><td class="py-2 px-4 border-b">{{ project.venue_address || 'N/A' }}</td></tr>
        <tr><td class="py-2 px-4 border-b font-medium">Start Date</td><td>:</td><td class="py-2 px-4 border-b">{{ project.start_date || 'N/A' }}</td></tr>
        <tr><td class="py-2 px-4 border-b font-medium">End Date</td><td>:</td><td class="py-2 px-4 border-b">{{ project.end_date || 'N/A' }}</td></tr>
        <tr><td class="py-2 px-4 border-b font-medium">Start Time</td><td>:</td><td class="py-2 px-4 border-b">{{ project.start_time || 'N/A' }}</td></tr>
        ...
        <tr>
          ...
          <td class="px-2 py-2 text-left">
            <div v-if="project.images && project.images.length" class="mt-2 grid grid-cols-2 md:grid-cols-3 gap-4">
              <img v-for="(img, index) in project.images" :key="img.id || index" :src="img.image_url" alt="Project Image" class="max-w-full rounded-lg" />
            </div>
