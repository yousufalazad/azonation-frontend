    ...
    try {
        ...
        if (response.status) {
            ...
            images.value = (project.images || []).map((image) => ({
                id: image.id,
                file: { preview: image.image_url || '', name: image.file_name || '' },
            }));
...
const submitForm = async () => {
    ...
    try {
        ...
        if (result.isConfirmed) {
            const response = await auth.uploadProtectedApi(`/api/projects/${projectId}`, formData, 'POST', {
            ...
            if (response.status) {
                ...
                Swal.fire('Failed!', response.errors?.message || 'Failed to update the project.', 'error');
            }
        }
    } catch (error) {
...
<template>
    <div class="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 py-4">
            <h2 class="text-xl sm:text-2xl font-bold text-gray-800">Edit Project</h2>
            <div class="flex flex-wrap gap-2">
                <button @click="$router.push({ name: 'view-individual-project', params: { id: projectId } })"
                    class="bg-green-500 hover:bg-green-600 text-white text-sm sm:text-base py-2 px-3 sm:px-4 rounded-lg shadow-md">
                    Project View
                </button>
        ...
        <form @submit.prevent="submitForm" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="col-span-full">
                    <label for="title"
                        class="block text-sm font-medium text-gray-700 after:content-['*'] after:text-red-500 after:ml-1">Project
                        Title</label>
                    <input v-model="title" type="text" id="title"
                        class="mt-2 w-full border border-gray-300 rounded-lg shadow-sm text-sm px-4 py-2" required />
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select v-model="status" id="status"
                        class="mt-2 w-full border border-gray-300 rounded-lg py-2 px-4 shadow-sm text-sm">
                ...
                <div class="space-y-3">
                    <div v-for="(file, index) in documents" :key="file.id"
                        ...
                        <div class="flex items-center gap-4 w-full md:w-auto">
                            <span v-if="file.file" class="truncate w-full md:w-32">{{ file.file.name }}</span>
                            <button type="button" class="bg-red-500 text-white px-2 py-1 text-sm hover:bg-red-600"
            ...
            <div class="flex justify-end gap-4">
                <button type="submit"
                    class="px-6 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 shadow">Save
                    Changes</button>
                <button type="button" @click="resetForm"
                    class="px-6 py-2 bg-gray-500 text-white font-medium rounded-lg hover:bg-gray-600 shadow">Reset</button>
            </div>
        </form>
    </div>
</template>