...
const submitForm = async () => {
  ...
  if (date.value) formData.append('date', date.value);
  if (time.value) formData.append('time', time.value);
  ...
  try {
    ...
    if (result.isConfirmed) {
      const response = await auth.uploadProtectedApi(`/api/events/${eventId}`, formData, 'POST', {
      ...
      if (response.status) {
        ...
        Swal.fire('Failed!', response.errors?.message || 'Failed to update event.', 'error');
      }
    }
  } catch (error) {
...
<template>
  <div class="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
      <h5 class="text-xl font-semibold text-center sm:text-left">Edit Event</h5>
    ...
    <form @submit.prevent="submitForm">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" id="title" v-model="title" class="w-full border border-gray-300 rounded-md py-2 px-4" required />
        </div>
        <div>
          <label for="name" class="block text-sm font-medium text-gray-700">Name</label>
          <input type="text" id="name" v-model="name" class="w-full border border-gray-300 rounded-md py-2 px-4" />
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input type="date" id="date" v-model="date" class="w-full border border-gray-300 rounded-md py-2 px-4" />
        </div>
        <div>
          <label for="time" class="block text-sm font-medium text-gray-700">Time</label>
          <input type="time" id="time" v-model="time" class="w-full border border-gray-300 rounded-md py-2 px-4" />
        </div>
        <div>
          <label for="venue_name" class="block text-sm font-medium text-gray-700">Venue Name</label>
          <input type="text" id="venue_name" v-model="venue_name" class="w-full border border-gray-300 rounded-md py-2 px-4" />
        </div>
        <div>
          <label for="venue_address" class="block text-sm font-medium text-gray-700">Venue Address</label>
          <input type="text" id="venue_address" v-model="venue_address" class="w-full border border-gray-300 rounded-md py-2 px-4" />
        </div>
        <div>
          <label for="requirements" class="block text-sm font-medium text-gray-700">Requirements</label>
          <input type="text" id="requirements" v-model="requirements" class="w-full border border-gray-300 rounded-md py-2 px-4" />
        </div>
        <div>
          <label for="note" class="block text-sm font-medium text-gray-700">Note</label>
          <input type="text" id="note" v-model="note" class="w-full border border-gray-300 rounded-md py-2 px-4" />
        </div>
        <div>
        ...
        <div>
          ...
          <select id="conduct_type" v-model="conduct_type" class="w-full border border-gray-300 rounded-md py-2 px-4">
      ...
      <div class="mb-4 mt-4">
        ...
        <button type="button" class="mt-3 w-full md:w-auto bg-blue-500 text-white py-1 px-3 rounded-md hover:bg-blue-700"
        ...
        <div class="space-y-3">
          <div v-for="(file, index) in documents" :key="file.id" class="flex flex-col md:flex-row gap-4">
            ...
            <div class="flex items-center gap-3 w-full md:w-auto">
              <span v-if="file.file" class="truncate md:w-32 w-full">{{ file.file.name }}</span>
              <button type="button" class="bg-red-500 text-white px-2 py-1 text-sm hover:bg-red-600"
      ...
      <div class="flex justify-end gap-4 mt-6">
        <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Update Event</button>
