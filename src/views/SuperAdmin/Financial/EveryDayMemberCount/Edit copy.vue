...
const getRecord = async () => {
  try {
    const response = await auth.fetchProtectedApi(`/api/get-every-day-member-count/${billingId}`, {}, 'GET');
    if (response.status) {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (result.isConfirmed) {
      const response = await auth.fetchProtectedApi(`/api/update-every-day-member-count/${billingId}`, payload, 'PUT');

      if (response.status) {
        Swal.fire('Success!', 'Billing updated successfully.', 'success').then(() => {
          router.push({ name: 'super-admin-every-day-member-count-list' });
        });
      } else {
