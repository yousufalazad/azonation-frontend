<template>
  <div class="p-6">
    ...
    <!-- Search Input -->
    <input v-model="searchQuery" @input="filterResults" type="text" placeholder="Search by user or package"
    ...
    <table class="table-auto w-full bg-white shadow rounded">
      ...
      <tbody>
        ...
        <tr>
          <td class="border px-4 py-2">{{ userPriceRates.package_id }}</td>
          <td class="border px-4 py-2">{{ userPriceRates.region }}</td>
          <td class="border px-4 py-2">{{ userPriceRates.price }}</td>
        </tr>
