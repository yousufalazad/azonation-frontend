...
const fetchLogo = async () => {
  try {
      const response = await auth.fetchProtectedApi(`/api/super_admin_profile_image/${userId}`, {}, 'GET');
...
const profileImageUpdate = async () => {
  if (selectedImage.value) {
      ...
      try {
          const imageResponse = await auth.uploadProtectedApi(`/api/super_admin_profile_image/${userId}`, formData);
