...
<template>
  <div class="d-flex flex-column align-items-center justify-content-center mt-5 px-3">
    <div class="w-75 bg-white rounded-lg shadow-lg sm:max-w-md px-5">
      ...
      <div class="mb-3">
        <label for="admin_name" class="form-label">SuperAdmin Name</label>
        <input v-model="admin_name" type="text" id="admin_name" class="form-control" placeholder="Name" required>
        <p v-if="auth.errors?.admin_name" class="text-danger mt-2">{{ auth.errors?.admin_name[0] }}</p>
      </div>
      <div class="mb-3">
      ...
      <div class="text-end pb-4">
        <button @click="auth.superAdminRegister(admin_name, email, password)" class="btn btn-primary">Super Admin Register</button>
      </div>
    </div>
  </div>
</template>
