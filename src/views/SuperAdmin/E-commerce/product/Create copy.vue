...
const submitForm = async () => {
  if (!name.value || !base_price.value || !sku.value) {
    ...
    if (result.isConfirmed) {
      ...
      if (response.status) {
        Swal.fire('Success!', 'Product created successfully.', 'success').then(() => {
          resetForm();
          router.push({ name: 'product-list' }); // Adjust route name
        });
      } else {
    ...
    <form @submit.prevent="submitForm">
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" id="business_type_id" v-model="business_type_id"
            class="border mt-1 border-gray-300 rounded-md p-2 block w-full" required />
        </div>
        <div>
        ...
        <div>
          ...
          <input type="text" id="sub_category_id" v-model="sub_category_id"
            class="border mt-1 border-gray-300 rounded-md p-2 block w-full" required />
        </div>
        <div>
          <label for="sub_sub_category_id" class="block text-sm font-medium text-gray-700">Sub Sub Category</label>
          <input type="text" id="sub_sub_category_id" v-model="sub_sub_category_id"
            class="border mt-1 border-gray-300 rounded-md p-2 block w-full" required />
        </div>
        <div>
          <label for="brand_id" class="block text-sm font-medium text-gray-700">Brand</label>
          <input type="text" id="brand_id" v-model="brand_id"
            class="border mt-1 border-gray-300 rounded-md p-2 block w-full" required />
        </div>
        <div>
        ...
        <div>
          ...
          <textarea id="short_description" v-model="short_description"
            class="border mt-1 border-gray-300 rounded-md p-2 block w-full" required></textarea>
        </div>
        <div>
