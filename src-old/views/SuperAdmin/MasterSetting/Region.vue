...
const submitForm = async () => {
    ...
    try {
        ...
        const result = await Swal.fire({
            title: 'Are you sure?',
            text: `Do you want to ${isEditMode.value ? 'update' : 'add'} this region?`,
            icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            if (response.status) {
                await Swal.fire('Success!', `region ${isEditMode.value ? 'updated' : 'added'} successfully.`, 'success');
                getRegions();
                resetForm();
            } else {
                Swal.fire('Failed!', 'Failed to save region.', 'error');
            }
        }
    } catch (error) {
        console.error(`Error ${isEditMode.value ? 'updating' : 'adding'} region:`, error);
        Swal.fire('Error!', `Failed to ${isEditMode.value ? 'update' : 'add'} region.`, 'error');
    }
};

// Edit region
const editCountry = (region) => {
    name.value = region.name;
    title.value = region.title;
    is_active.value = region.is_active;
    ...
    try {
        ...
        if (result.isConfirmed) {
            ...
            if (response.status) {
                ...
                Swal.fire('Failed!', 'Failed to delete region.', 'error');
            }
        }
    } catch (error) {
    ...
    <div class="max-w-7xl mx-auto w-10/12">
        <section class="mb-5">
            <div class="flex justify-between left-color-shade py-2 my-3">
                <h5 class="text-md font-semibold mt-2">{{ isEditMode ? 'Edit' : 'Add' }} Region</h5>
            </div>
        ...
        <section>
            <div class="flex justify-between left-color-shade py-2 my-3">
                <h5 class="text-md font-semibold mt-2">Region List</h5>
            </div>
            <table class="min-w-full table-auto border-collapse border border-gray-300 text-left">
                ...
                <tbody>
                    <tr v-for="(region, index) in RegionList" :key="region.id">
                        <td class="py-2 px-4 border">{{ index + 1 }}</td>
                        <td class="py-2 px-4 border">{{ region.name }}</td>
                        <td class="py-2 px-4 border">{{ region.title }}</td>
                        <td class="py-2 px-4 border">
                            <span :class="region.is_active === 0 ? 'text-red-500' : 'text-green-500'">
                                {{ region.is_active === 0 ? "No" : "Yes" }}
                            </span>
                        </td>
                        <td class="py-2 px-4 border flex justify-end gap-2">
                            <button @click="editCountry(region)"
                                class="bg-yellow-400 text-white rounded-md py-1 px-2 mx-2 hover:bg-yellow-500">Edit</button>
                            <button @click="deleteCountry(region.id)"
