...
const getRecords = async () => {
  try {
    const response = await auth.fetchProtectedApi(`/api/every-day-member-count-and-bill-list`, {}, 'GET');
...
const deleteRecord = async (id) => {
  try {
    ...
    if (confirmed.isConfirmed) {
      const response = await auth.fetchProtectedApi(`/api/delete-every-day-member-count-and-bill/${id}`, {}, 'DELETE');
      if (response.status) {
  ...
  <div class="max-w-7xl mx-auto w-10/12">
    <section>
      <div class="flex justify-between left-color-shade py-2 my-3">
        <h5 class="text-md font-semibold mt-2">Every Day Member Count and Bill List</h5>
        <div>
          <button @click="$router.push({ name: 'super-admin-every-day-member-count-and-bill-create' })"
          ...
          <table class="min-w-full bg-white border border-gray-200">
            <thead class="bg-gray-50">
              <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                ...
                <th class="py-2 px-4 border w-48">Day Total Member</th>
                <th class="py-2 px-4 border w-48">Day Total Bill</th>
                <th class="py-2 px-4 border w-20">Active</th>
                <th class="border px-1 py-3 text-left w-72">Action</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200">
              <!-- {{ everyDayMemberCountList }} -->
              <tr v-for="(data, index) in everyDayMemberCountList" :key="data.id">
                ...
                <td class="py-2 px-4 border">
                  <button
                    @click="$router.push({ name: 'super-admin-every-day-member-count-and-bill-edit', params: { id: data.id } })"
                    class="bg-yellow-500 hover:bg-yellow-600 text-white px-2 py-1 m-2 rounded">Edit</button>
                  <button
                    @click="$router.push({ name: 'super-admin-every-day-member-count-and-bill-view', params: { id: data.id } })"
      ...
      <div v-else class="p-4">
        <p class="text-center text-gray-500">No Every Day Member Count available. Click "Create Every Day Member Count"
          to add a new one.</p>
      </div>
    </section>
  </div>
