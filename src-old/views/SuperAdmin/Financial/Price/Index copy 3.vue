  ...
  <div class="container mx-auto p-6">
      ...
      <table class="min-w-full">
        ...
        <tbody>
          <tr v-for="tierIndex in 20" :key="tierIndex">
            <td class="py-2 px-4 border">Region {{ tierIndex }}</td>
            <td v-for="priceRate in priceRates" :key="priceRate.id" class="py-2 px-4 border">
              {{ priceRate[`tier${tierIndex}`] }}
            </td>
            <td class="py-2 px-4 border text-center">
              <button @click="openEditModal(tierIndex)" class="bg-blue-600 text-white px-3 py-1 rounded-md hover:bg-blue-700">
                Edit
              </button>
            </td>
          </tr>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white rounded-lg p-6 w-full max-w-md">
        <h2 class="text-xl font-semibold mb-4">Edit Prices for Tier {{ editingTier }}</h2>
        ...
        <div v-for="priceRate in priceRates" :key="priceRate.id" class="mb-3">
          ...
          <input
            v-model="priceRate[`tier${editingTier}`]"
            type="number"
...
const openEditModal = (tierIndex) => {
  editingTier.value = tierIndex;
